...
class Base_Task(gym.Env):
    ...
    def create_table_and_wall(self, table_xy_bias=[0, 0], table_height=0.74):
        ...
        if self.random_background:
            ...
            directory_path = os.path.join(ROOT_PATH , f"assets/background_texture/{texture_type}")
            file_count = len(
    ...
    def take_action(self, action, action_type='qpos'):  # action_type: qpos or ee
        if self.take_action_cnt == self.step_lim:
            print(f"Should not reach here self.take_action_cnt == self.step_lim", flush=True)
            return
        ...
        self.take_action_cnt += action.shape[0]
        #print(f"step: \033[92m{self.take_action_cnt} / {self.step_lim}\033[0m", end="\r")
