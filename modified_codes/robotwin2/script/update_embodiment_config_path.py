...
def main():
    ...
    if os.path.exists(embodiment_config_path):
        try:
            ...
            if original_content != content:
                ...
                for embodiment in embodiments:
                    if embodiment in content:
                        print(f"    - Updated path for {embodiment}")
            else:
                print_color("  ✓ _embodiment_config.yml already up to date", GREEN)
                
        except Exception as e:
            print_color(f"  ✗ Failed to process _embodiment_config.yml: {e}", YELLOW)
    else:
        print_color(f"  ! Warning: _embodiment_config.yml not found at {embodiment_config_path}", YELLOW)
