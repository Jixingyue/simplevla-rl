...
class SPMDGPUExecutor(ExecutorBase):
    ...
    def determine_num_available_blocks(self) -> Tuple[int, int]:
        ...
        此方法会在每个 worker 上调用 `determine_num_available_blocks`，并取
        结果中的最小值，以保证所选的 cache 大小对所有 worker 都兼容。
        ...
        # NOTE(shengguangming): 现在我们不使用共享的集中式控制器，而是每个进程
        # 都拥有自己的 scheduler
    ...
    def initialize_cache(self, num_gpu_blocks: int, num_cpu_blocks: int) -> None:
        ...
        # NOTE: 我们在这里打日志，以避免 worker 数量大于一时产生多条重复日志。
        # 我们也可以在 engine 中打日志，但并非所有 executor 都有 GPU。
    ...
    # NOTE(sgm): 重建 KVCache 时不会对模型进行 profile 和捕获（CUDAGraph）
...
def initialize_cluster(
    ...
    port = get_open_port()
    # 我们需要设置分布式初始化方法，以确保
    # 分布式的 megatron 代码（例如获取 world size）能正常工作。
